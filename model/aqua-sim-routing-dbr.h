...
class QueueItem2 : public Object {
public:
	QueueItem2() : /*m_p(0),*/ m_sendTime(0) {}
	QueueItem2(Ptr<Packet> p, double t) : m_p(p), m_sendTime(t) {}
...
public:
	...
	QueueItem2* front() { return m_dq.front(); };
	void insert(QueueItem2* q);
	bool update(Ptr<Packet> p, double t);
	bool purge(Ptr<Packet> p);

private:
	std::deque<QueueItem2*> m_dq;
};  // class MyPacketQueue
